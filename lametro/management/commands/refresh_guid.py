...
class ClassificationMixin:
    ...
    FACET_CLASSES = {
        ...
        "phase_exact": ("Transportation Phase",),
        "project_exact": (
            "Project",
            "Project Development",
            "Project Finance",
        ...
        "significant_date_exact": ("Date",),
        "motion_by_exact": ("Board Member",),
        "plan_program_policy_exact": ("Plan", "Program", "Policy"),
    }
                ...
                *[
                    {
                        subject: facet
                        for subject in list(
                            self.get_subjects_from_classes(facet, classes)
                        )
                    }
...
class Command(BaseCommand, ClassificationMixin):
    ...
    def handle(self, *args, **options):
        ...
        LAMetroSubject.objects.bulk_create(
            [
                LAMetroSubject(name=s, classification=self.DEFAULT_FACET)
        ...
        for topic in self.legistar.topics():
            try:
                ...
                subject.classification = self.classifications.get(
                    subject.name, self.DEFAULT_FACET
                )
        ...
        try:
            ...
            self.stdout.write(
                "Updated only {0} of {1} total topics".format(update_count, topic_count)
            )
        else:
