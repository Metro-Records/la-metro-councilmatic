...
def get_alternate_files(entity, type, friendly_name) -> dict:
    """
    Get urls for all language + file combinations
    of a bill or event document found in S3.

    :return result: A dict with languages as keys,
    and lists of urls to different file types as values.
    """

    if type not in ["bill", "event"]:
        raise ValueError("'type' string must be either 'bill' or 'event")

    base_url = (
        f"https://{settings.AWS_TRANSLATION_BUCKET_NAME}"
        + ".s3.amazonaws.com/approved/"
    )
    file_types = [".pdf", ".rtf", ".md"]
    languages = ["english", "spanish"]
    result = {lang: [] for lang in languages}

    if type == "bill":
        if source_document := entity.board_report:
            result["english"].append(source_document.url)
    elif type == "event":
        # Prevent importing the service before the app is ready
        from lametro.services.event_service import EventService

        if source_document := EventService.get_agenda(entity):
            result["english"].append(source_document["url"])

    if not source_document:
        # If the source document doesn't exist, then we don't have alternates
        return result

    # Build urls and check if documents are available
    for lang in languages:
        for ext in file_types:
            if not (lang == "english" and ext == ".pdf"):
                # TODO: nail down the naming convention for files and then test
                url = base_url + f"{lang}/" + friendly_name + ext

                response = requests.head(url)
                result[lang].append(url) if response.status_code == 200 else None
